...
_gtkextra_signal_emit(GtkObject *object, guint signal_id, ...)
{
  ...
  for (i = 0; i < (gint) query.n_params; i++)
    {
  ...
  if(state == GTK_SHEET_NORMAL)
    {
      ...
      g_signal_connect (gtk_sheet_get_entry (sheet), "changed",
                        G_CALLBACK (gtk_sheet_entry_changed), sheet);
    }
  ...
  if (sheet->vadjustment)
    {
      ...
      g_signal_connect (sheet->vadjustment, "changed",
                        G_CALLBACK (vadjustment_changed), sheet);
      g_signal_connect (sheet->vadjustment, "value_changed",
                        G_CALLBACK (vadjustment_value_changed), sheet);
    }
  ...
  if (sheet->hadjustment)
    {
      ...
      g_signal_connect (sheet->hadjustment, "changed",
                        G_CALLBACK (hadjustment_changed), sheet);
      g_signal_connect (sheet->hadjustment, "value_changed",
                        G_CALLBACK (hadjustment_value_changed), sheet);
    }
...
create_global_button(GtkSheet *sheet)
{
   ...
   g_signal_connect (sheet->button, "pressed",
                     G_CALLBACK (global_button_clicked), sheet);
}
...
gtk_sheet_draw_backing_pixmap(GtkSheet *sheet, GtkSheetRange range)
{
  ...
  width = MIN (width, (gint) (sheet->sheet_window_width - x));
  height = MIN (height, (gint) (sheet->sheet_window_height - y));
                   ...
                   const gchar *text)
{
 ...
 if(attributes.is_visible){
   ...
   if(gtk_sheet_autoresize(sheet) &&
      (text_width > (gint) (sheet->column[col].width - 2 * CELLOFFSET - attributes.border.width))) {
...
gtk_sheet_deactivate_cell(GtkSheet *sheet)
{
 ...
 g_signal_handlers_disconnect_by_func (gtk_sheet_get_entry (sheet),
                                       (gpointer) gtk_sheet_entry_changed,
                                       sheet);
		  ...
		  GdkEventExpose * event)
{
  ...
  g_debug ("==== Enter gtk_sheet_expose()\n"
           "Must have received expose_event\n");
...
gtk_sheet_move_query(GtkSheet *sheet, gint row, gint column)
{
  ...
  if(row>=MAX_VISIBLE_ROW(sheet) && sheet->state!=GTK_SHEET_COLUMN_SELECTED) {
	  ...
	  new_row = MIN(sheet->maxrow, row + 1);
          ...
          if(MAX_VISIBLE_ROW(sheet) == sheet->maxrow &&
             ...
             sheet->row[sheet->maxrow].height < (gint) height) {
                 row_move = FALSE;
		 row_align = -1.;
          }
  }
  ...
  if(column>=MAX_VISIBLE_COLUMN(sheet) && sheet->state!=GTK_SHEET_ROW_SELECTED) {
          ...
          if(MAX_VISIBLE_COLUMN(sheet) == sheet->maxcol &&
             ...
             sheet->column[sheet->maxcol].width < (gint) width) {
                 column_move = FALSE;
		 col_align = -1.;
          }
  } 
...
*/
  ...
  case GDK_KEY_Left:
    ...
    default:
      g_debug ("gtk_sheet_key_press: "
               "After switch, found default case.\n"
               "  User probably typed letter key or DEL.\n");
      if(in_selection) {
	GTK_SHEET_SET_FLAGS(sheet, GTK_SHEET_IN_SELECTION);
...
create_sheet_entry(GtkSheet *sheet)
{
 ...
 g_signal_connect_swapped (entry, "key_press_event",
                           G_CALLBACK (gtk_sheet_key_press), sheet);
				     ...
				     gpointer data)
{
  ...
  for(i=0; i<= sheet->maxrow; i++){
   ...
   if(y > gtk_adjustment_get_value (adjustment)) break;
  }
...
/* Negative old_adjustment enforces the redraw, otherwise avoid spureous redraw */
  ...
  if(new_column == 0){
    ...
    gtk_adjustment_set_step_increment (sheet->hadjustment,
                                       (gdouble) MIN(sheet->column[new_column].width,
                                                     sheet->column[new_column-1].width));
  }
 ...
 while(children)
   {
     ...
     if (child->attached_to_cell && child->col > (gint) col) child->col -= ncols;
     children = g_list_next(children);
   }
  ...
  *requisition = button_requisition.height;
  ...
  while(children){
    ...
    if(child->attached_to_cell && child->row == row && child->col != -1 && !child->floating && !child->yshrink){
      ...
      if (child_requisition.height + 2 * child->ypadding > (gint) (*requisition))
  ...
  *requisition = button_requisition.width;
  ...
  while(children){
    ...
    if(child->attached_to_cell && child->col == col && child->row != -1 && !child->floating && !child->xshrink){
      ...
      if (child_requisition.width + 2 * child->xpadding > (gint) (*requisition))
