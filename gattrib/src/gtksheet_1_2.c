...
#define DEFAULT_ROW_HEIGHT(widget) (widget->style->font->ascent + 2*widget->style->font->descent + 2*CELLOFFSET)  
#define DEFAULT_LABEL_HEIGHT(widget) (widget->style->font->ascent+\
...
static void label_size_request                  (GtkSheet *sheet, gchar *label, 
						 GtkRequisition *req);
...
gtk_sheet_class_init (GtkSheetClass * klass)
{
  ...
  GtkType object_class_type;   /* SDB addition for GTK-2.X  */
  ...
  /* This was introduced by SDB in order to enable compilation under GTK-2.X */

  object_class_type = object_class->type; 
...
gtk_sheet_row_button_add_label(GtkSheet *sheet, gint row, const gchar *label)
{
  ...
  if(button->label && strlen(button->label)>0){
           ...
           while(words && *words != '\0'){
             if(*words == '\n' || *(words+1) == '\0'){
               label_height += DEFAULT_LABEL_HEIGHT(GTK_WIDGET(sheet));
             }
             words++;
           }
  }
...
gtk_sheet_clip_range (GtkSheet *sheet, const GtkSheetRange *range)
{
  ...
  sheet->interval=0;
  sheet->clip_timer=gtk_timeout_add(TIMEOUT_FLASH, gtk_sheet_flash, (gpointer) sheet); 
...
gtk_sheet_entry_changed(GtkWidget *widget, gpointer data)
{
 ...
 text = (gchar *) gtk_entry_get_text(GTK_ENTRY(gtk_sheet_get_entry(sheet)));
...
gtk_sheet_show_active_cell(GtkSheet *sheet)
{
 ...
 if(!text) text = (gchar *) g_strdup("");

 if(!GTK_IS_ITEM_ENTRY(sheet_entry))
    gtk_entry_set_text(GTK_ENTRY(sheet_entry), text);
 else
    gtk_item_entry_set_text(GTK_ITEM_ENTRY(sheet_entry), (const gchar *) text, justification);
			...
			GdkEventButton * event)
{
  ...
  /* selections on the sheet */
    if(event->window == sheet->sheet_window){
     ...
     sheet->timer=gtk_timeout_add(TIMEOUT_SCROLL, gtk_sheet_scroll, (gpointer) sheet); 
     GTK_WIDGET_UNSET_FLAGS(sheet->sheet_entry, GTK_HAS_FOCUS);
...
gtk_sheet_size_allocate_entry(GtkSheet *sheet)
{
 ...
 const gchar *text;  /* Needed for Gtk-2.X */
...
/* I'm emulating gtk_widget_size_request to avoid the redraw */
 ...
 /* SDB says: we may need to edit this for gtk-2  */
 ...
 /*  SDB says:  We may remove this (or rewrite it) for GTK22  */
 if(sheet->sheet_entry_window){
...
create_sheet_entry(GtkSheet *sheet)
{
 ...
 /* --- SDB says:  This doesn't appear in the Gtk-2.X version  --- */  
...
gtk_sheet_button_draw (GtkSheet *sheet, gint row, gint column)
{
  ...
  if(button->label_visible){
    ...
    text_height=DEFAULT_LABEL_HEIGHT(GTK_WIDGET(sheet));
    
    y += DEFAULT_ROW_HEIGHT(GTK_WIDGET(sheet))/2+
         DEFAULT_FONT_ASCENT(GTK_WIDGET(sheet))/2; 
    ...
    if(button->label && strlen(button->label)>0){
           ...
           while(words && *words != '\0'){
             ...
             if(*words == '\n' || *(words+1) == '\0'){
	       text_width = gdk_string_width (GTK_WIDGET (sheet)->style->font, line);
               ...
               y += DEFAULT_FONT_ASCENT(GTK_WIDGET(sheet))+
                    DEFAULT_FONT_DESCENT(GTK_WIDGET(sheet)) + 2;
	   ...
	   text_width = gdk_string_width (GTK_WIDGET (sheet)->style->font, label);
...
label_size_request(GtkSheet *sheet, gchar *label, GtkRequisition *req)
{
  ...
  while(words && *words != '\0'){
    if(*words == '\n' || *(words+1) == '\0'){
      ...
      word[n] = '\0';
      req->width = MAX( req->width, gdk_string_width (GTK_WIDGET (sheet)->style->font, word) );
      n = 0;
    } else {
      word[n++] = *words;
    }
                                 ...
                                 GtkRequisition *button_requisition)
{
  ...
  if(button->child)
  {
    ...
    /* taken from 2.2 */
    requisition.width += 2;       /* 2*button->child->xpadding;  */
    requisition.height += 2;      /* 2*button->child->ypadding;  */
    requisition.width += 2*sheet->button->style->klass->xthickness;
    requisition.height += 2*sheet->button->style->klass->ythickness;
  } else {
