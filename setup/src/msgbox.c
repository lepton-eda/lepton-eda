...
int MsgBox(char *szLabel, char **pButtonLabel)
{
	GtkDialog *pDialog;
	GtkLabel *pLabel;
	GtkHBox *pHBox;
	...
	switch ((int) pButtonLabel)
	{
		...
		default:
			...
			for (iNumber = 0; pButtonLabel[iNumber] != NULL && iNumber < MSGBOX_MAXBUTTONS; iNumber ++)
				pButton[iNumber] = GTK_BUTTON(gtk_button_new_with_label(pButtonLabel[iNumber]));
	}
