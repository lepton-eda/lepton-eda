...
(define spice-sdb:write-top-header
  (lambda (port)
    ...
    (display "* spice-sdb version 3.31.2003 by SDB --                 *\n" port)
...
(define spice-sdb:write-subcircuit-header
  (lambda (port)
    ...
    (display "* spice-sdb ver 3.31.2003     *\n" port)
    (display "*******************************\n" port)
  )
)
...
;;--------------------------------------------
    ...
    ;; now write out netlist
      (spice-sdb:write-netlist port packages) ;; does "packages" get executed automatically???
