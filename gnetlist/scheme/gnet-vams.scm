  ...
  (lambda (attribute connlist)
    (begin
	  ...
	  (if (= (length connlist) 1)
		  ...
		  (if (equal? (gnetlist:get-pin-attribute2 (car (car connlist)) 
							  (car (cdr (car connlist)))
							  attribute)
			      'out)
		      #t
		      #f)
		  (append (gnetlist:get-pin-attribute2 (car (car connlist)) 
						      (car (cdr (car connlist)))
						      attribute)))
	      (if (equal? attribute 'port_mode)
		  (if (equal? (gnetlist:get-pin-attribute2 (car (car connlist))  
    ...
    (let ((port-list  (list '())))
	  ...
	  (begin
	    (for-each (lambda (pin)
			(append! port-list
				 (list (list pin
					     (gnetlist:get-pin-attribute2 uref pin "port_object")
					     (gnetlist:get-pin-attribute2 uref pin "port_type")
					     (gnetlist:get-pin-attribute2 uref pin "port_mode")))))
