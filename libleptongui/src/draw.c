 ...
 *  \par Function Description
 *
 *  Draws a net using the variables defined in the current
 *  #SchematicWindow instance and all other data set in \p
 *  renderer.
 *
 *  If the magnetic net mode is set, the function may draw the
 *  magnetic net connection marker.
 *
 *  \param [in] w_current The #SchematicWindow instance.
 *  \param [in] renderer The renderer.
 */
 ...
 * \par Function Description
 *
 *  Draws a preview of the path currently being drawn, including a
 *  helper line showing the control point of the node being drawn
 *  (if applicable).  The path is drawn using the variables
...
/*! \brief Draw a bounding box or outline for \c LeptonObject
 *         placement
 *
 *  \par Function Description
 *
 *  This function draws either the objects in the place list or a
 *  rectangle around their bounding box, depending upon the
 *  currently selected \a w_current->actionfeedback_mode. This
 *  takes the value \a BOUNDINGBOX or \a OUTLINE.
 *
 *  The function applies manhatten mode constraints to the
 *  coordinates before drawing if the CONTROL key is recording as
 *  being pressed in the \p w_current structure.
 *
 *  \param w_current The #SchematicWindow which we're drawing for.
...
#endif
{
  ...
  if (w_current->fast_mousepan &&
      schematic_window_get_current_canvas (w_current)->doing_pan)
    ...
    if (schematic_window_get_rubber_visible (w_current))
    {
      switch (action_mode)
      {
        ...
        case NETMODE:
          ...
          schematic_draw_net (w_current, renderer);
