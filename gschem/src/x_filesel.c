	...
	if (!w_current->fowindow) {
...
#if 0
		gtk_file_selection_set_filename(w_current->fowindow,
						COMPONENT_LIBRARY);
#endif

		gtk_window_position(GTK_WINDOW(w_current->fowindow),
				    GTK_WIN_POS_MOUSE);
		/* added 4/6/98 */
		...
		gtk_signal_connect(GTK_OBJECT(
			GTK_FILE_SELECTION(w_current->fowindow)->ok_button),
				   "clicked",
				   GTK_SIGNAL_FUNC(file_selection_ok_open),
				   w_current);
		...
		gtk_signal_connect(GTK_OBJECT(
			...
			cancel_button),
				    "clicked",
				   GTK_SIGNAL_FUNC(file_selection_cancel_open),
				    w_current);
...
file_selection_ok_saveas (GtkWidget *w, TOPLEVEL *w_current)
{
	...
	string = gtk_file_selection_get_filename(
		GTK_FILE_SELECTION(w_current->fswindow));
	...
	if (w_current->saveas_flag == QUIT) {
		...
		/* commented out 5/9 */
#if 0
		i_callback_file_quit(NULL, w_current);
#endif
        }
...
file_selection_cancel_saveas (GtkWidget *w, TOPLEVEL *w_current)
{
	 ...
	 * or maybe not? NOT, since pages are replaced, new ones added
	 * only */
	if (w_current->saveas_flag == NEW) {
		w_current->page_current->CHANGED = 0;
		i_callback_file_new(w_current, 0, NULL);
	...
	w_current->saveas_flag = flag;
  	...
  	if (!w_current->fswindow) {
		...
		gtk_file_selection_set_filename(
			GTK_FILE_SELECTION(w_current->fswindow),
			filename);
		gtk_window_position(GTK_WINDOW(w_current->fswindow),
				    GTK_WIN_POS_MOUSE);
		/* added 4/6/98 */
		gtk_file_selection_hide_fileop_buttons(
			GTK_FILE_SELECTION(w_current->fswindow));
		...
		gtk_signal_connect(GTK_OBJECT(
			GTK_FILE_SELECTION(w_current->fswindow)->ok_button),
				   "clicked",
				   GTK_SIGNAL_FUNC(file_selection_ok_saveas),
				   w_current);
