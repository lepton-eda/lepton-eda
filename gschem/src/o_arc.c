...
o_arc_draw(TOPLEVEL *w_current, OBJECT *o_current)
{
	...
	world_get_arc_bounds(w_current, o_current, &wleft, &wtop, &wright, &wbottom);
...
o_arc_draw_xor(TOPLEVEL *w_current, int dx, int dy, OBJECT *o_current)
{
	...
	width= (o_current->line_points->screen_x1+dx) - (o_current->screen_x+dx);
	height= (o_current->line_points->screen_y1+dy) - (o_current->screen_y+dy);
...
/* out because it erases the background incorrectly... all you need is above  
	gdk_gc_set_foreground(w_current->gc, 
...
*/
	...
	last_y = snap_grid(w_current, last_y); commented out*/
...
o_arc_end2(TOPLEVEL *w_current, int start_angle, int end_angle)
{
	...
	w_current->page_current->object_tail = o_arc_add(w_current, w_current->page_current->object_tail, 
