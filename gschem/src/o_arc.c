...
void o_arc_draw(TOPLEVEL * w_current, OBJECT * o_current)
{
    ...
    (*draw_func) (w_current->window, w_current->gc, color,
		  ...
		  o_current->arc->end_angle, arc_width, length, space);
...
o_arc_draw_solid(GdkWindow * w, GdkGC * gc, GdkColor * color,
		 GdkCapStyle cap, gint x, gint y, gint width, gint height,
...
o_arc_draw_phantom(GdkWindow * w, GdkGC * gc, GdkColor * color,
		   GdkCapStyle cap, gint x, gint y, gint width,
		   gint height, gint angle1, gint angle2, gint arc_width,
...
void o_arc_end2(TOPLEVEL * w_current, int start_angle, int end_angle)
{
	...
	o_arc_add(w_current, w_current->page_current->object_tail,
		  ...
		  x1, y1, x2, y2, start_angle * 64, end_angle * 64);

    (*w_current->page_current->object_tail->draw_func) (w_current,
							w_current->
							page_current->
							object_tail);
    w_current->start_x = (-1);
