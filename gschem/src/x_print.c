...
x_print_print(GtkWidget *w, TOPLEVEL *w_current)
{
	...
	filename = gtk_entry_get_text(GTK_ENTRY(w_current->pfilename_entry));
...
x_print_setup (TOPLEVEL *w_current, char *filename)
{
	...
	if (!w_current->pwindow) {
		...
		gtk_signal_connect (GTK_OBJECT (w_current->pwindow), "destroy",
			GTK_SIGNAL_FUNC(destroy_window),
			&w_current->pwindow);

		gtk_signal_connect (GTK_OBJECT (w_current->pwindow), "delete_event",
			GTK_SIGNAL_FUNC(destroy_window),
			&w_current->pwindow);

		gtk_window_set_title (GTK_WINDOW (w_current->pwindow), "Print...");

		buttonprint = gtk_button_new_with_label ("Print");
		GTK_WIDGET_SET_FLAGS (buttonprint, GTK_CAN_DEFAULT);
		gtk_box_pack_start (GTK_BOX (
			GTK_DIALOG(w_current->pwindow)->action_area),
		...
		box = gtk_hbox_new(FALSE, 0);
        	...
        	gtk_container_add(GTK_CONTAINER(GTK_DIALOG(w_current->pwindow)->vbox), box);
        	gtk_widget_show(box);            
		...
		w_current->pfilename_entry = gtk_entry_new_with_max_length (79);
                gtk_editable_select_region (GTK_EDITABLE (w_current->pfilename_entry), 0, -1);
                gtk_box_pack_start (GTK_BOX (box),
...
/* delete this eventually */
		...
		button = gtk_radio_button_new_with_label (group, "Current Window");
  		gtk_box_pack_start (GTK_BOX (GTK_DIALOG (w_current->pwindow)->vbox), 
			button, TRUE, TRUE, 0);
		/* rewrite callbacks to use third parameter to be clean hack */
		gtk_signal_connect(GTK_OBJECT (button), "clicked",
...
/* radio end */
		...
		gtk_box_pack_start (GTK_BOX (GTK_DIALOG (w_current->pwindow)->vbox), optionmenu, TRUE, TRUE, 0);
	...
	if (!GTK_WIDGET_VISIBLE (w_current->pwindow)) {
        	gtk_entry_set_text(GTK_ENTRY(w_current->pfilename_entry), filename);
        	/*gtk_entry_select_region(GTK_ENTRY(w_current->pfilename_entry), 0, strlen(filename)); 	*/
