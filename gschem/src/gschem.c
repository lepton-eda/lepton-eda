...
gschem_quit(void)
{
	...
	/* x_window_free_head(); can't do this since it causes a condition */
	/* in which window_head->... is still being refered after this */
...
main_prog(int argc, char *argv[])
{
	int i;
	TOPLEVEL *w_current;
	char input_str[256]; /* size is hack */
	int argv_index;
        int first_page=1;
...
if (!quiet_mode) {
	...
	while (argv[i] != NULL) {
		...
		/* for the first page */
			...
			/* this needed so that display is shown properly */
