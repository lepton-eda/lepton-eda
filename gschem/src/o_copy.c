...
o_copy_end(TOPLEVEL *w_current)
{
		...
		switch(found->type) {
			case(OBJ_LINE):
				new_object = (OBJECT *) o_line_copy(
						w_current, w_current->page_current->object_tail, found);	
				if (w_current->actionfeedback_mode == OUTLINE) {
					o_line_draw_xor(w_current, screen_diff_x, screen_diff_y, found);
				}
				o_line_translate_world(w_current, diff_x, diff_y, new_object);
			...
			case(OBJ_NET):
				...
				o_net_translate_world(w_current, diff_x, diff_y, new_object);
				...
				selection_list = (OBJECT *) o_list_copy_to(w_current, selection_list, new_object, SELECTION);	
			break;
			...
			case(OBJ_BOX):
				...
				o_box_translate_world(w_current, diff_x, diff_y, new_object);
			...
			case(OBJ_CIRCLE):
				...
				o_circle_translate_world(w_current, diff_x, diff_y, new_object);
			...
			case(OBJ_COMPLEX):
				...
				new_object = (OBJECT *) o_complex_copy_embedded(
						w_current, 
				...
				o_complex_world_translate_toplevel(w_current, diff_x, diff_y, new_object);
				...
				selection_list = (OBJECT *) return_tail(selection_list);
			...
			case(OBJ_PIN):
				...
				o_pin_translate_world(w_current, diff_x, diff_y, new_object);
			...
			case(OBJ_ARC):
				...
				o_arc_translate_world(w_current, diff_x, diff_y, new_object);
        ...
        if (found->attached_to) {
                if (found->attached_to->copied_to) {
                        o_attrib_attach(w_current,
                                ...
                                new_object, found->attached_to->copied_to);     

		/* I have no idea if this is really needed.... ? hack */
				/*o_attrib_slot_update(w_current, found->attached_to->copied_to);*/
				...
				o_ntext_translate_world(w_current, diff_x, diff_y, new_object);

				/* old object was attr */
				if (!new_object->attribute && found->attribute) { 
					new_object->color = w_current->detachedattr_color; 
					o_complex_set_color(w_current, new_object->color, new_object);
					new_object->visibility = VISIBLE;
				}

				/* signify */
				/* that object is no longer an attribute */
	...
	/* hack this slows down the unselect */
