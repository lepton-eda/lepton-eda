...
void o_circle_draw(TOPLEVEL * w_current, OBJECT * o_current)
{
    ...
    (*draw_func) (w_current->window, w_current->gc, color,
		  ...
		  2 * radius, 2 * radius,
		  0, FULL_CIRCLE, circle_width, length, space);
    ...
    (*fill_func) (w_current->window, w_current->gc, color,
		  ...
		  radius, fill_width, angle1, pitch1, angle2, pitch2);
		   ...
		   gint angle1, gint pitch1, gint angle2, gint pitch2)
{
    ...
    gdk_draw_arc(w, gc,
		 TRUE, x - radius, y - radius, 2 * radius, 2 * radius, 0,
		 FULL_CIRCLE);
		   ...
		   gint angle1, gint pitch1, gint angle2, gint pitch2)
{
    o_circle_fill_hatch(w, gc, color,
			x, y, radius, width, angle1, pitch1, -1, -1);
    o_circle_fill_hatch(w, gc, color,
			x, y, radius, width, angle2, pitch2, -1, -1);
...
void o_circle_start(TOPLEVEL * w_current, int x, int y)
{
    ...
    gdk_draw_arc(w_current->window, w_current->xor_gc, FALSE,
		 w_current->start_x,
		 w_current->start_y, 1, 1, 0, FULL_CIRCLE);
#endif
...
void o_circle_end(TOPLEVEL * w_current, int x, int y)
{
    ...
    gdk_draw_arc(w_current->window, w_current->xor_gc, FALSE,
		 ...
		 w_current->distance * 2, 0, FULL_CIRCLE);
    ...
    SCREENtoWORLD(w_current,
		  w_current->start_x,
		  w_current->start_y, &center_x, &center_y);
...
void
o_circle_draw_grips(TOPLEVEL * w_current, GdkWindow * w,
		    OBJECT * o_current)
{
    int size, x2size;
    int x, y;
