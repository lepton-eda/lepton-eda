...
x_grid_draw(TOPLEVEL *w_current)
{
	...
	gdk_gc_set_foreground(w_current->gc, x_get_color(w_current->grid_color));
		...
		for (j = y_start; j < w_current->page_current->bottom; j = j + incr) {
			...
			if (inside_region(w_current->page_current->left, 
				...
				if (count == 5000) {
					...
					gdk_draw_points(w_current->window, w_current->gc, points, count);
					gdk_draw_points(w_current->backingstore, w_current->gc, points, count);
					count=0;
