...
/* Do NOT use the widget parameter */
/* It is set to NULL most of the time */
void 
update_page_manager(GtkWidget *widget, TOPLEVEL *w_current)
{
	...
	gtk_clist_thaw(GTK_CLIST(w_current->page_clist));
        ...
        w_current->clist_sig = gtk_signal_connect (GTK_OBJECT (
...
setup_page_selector (TOPLEVEL *w_current)
{
	...
	if (!w_current->pswindow) {
		...
		gtk_container_border_width (GTK_CONTAINER (w_current->pswindow), 0);
		gtk_widget_set_usize(w_current->pswindow, 500, 220);
	
		gtk_window_position (GTK_WINDOW (w_current->pswindow), GTK_WIN_POS_NONE);

		gtk_signal_connect (GTK_OBJECT (w_current->pswindow), "destroy",
			GTK_SIGNAL_FUNC(destroy_window),
			&w_current->pswindow);

		gtk_signal_connect (GTK_OBJECT (w_current->pswindow), "delete_event",
			GTK_SIGNAL_FUNC(destroy_window),
			&w_current->pswindow);

		gtk_window_set_title (GTK_WINDOW (w_current->pswindow), "Page Manager");
...
/* gtk_clist_set_policy is gone in gtk 1.2.x */
		...
		gtk_clist_set_column_justification (GTK_CLIST (
				w_current->page_clist), 1, GTK_JUSTIFY_CENTER);
		...
		/* we need to store the signal handler here */
		/* since we disconnect it and then reconnect it later */
       		w_current->clist_sig = gtk_signal_connect (GTK_OBJECT (
			   w_current->page_clist),
                          "select_row",
                           GTK_SIGNAL_FUNC(change_page),
		...
		row = s_page_search_row(w_current, w_current->page_current);
		gtk_clist_select_row(GTK_CLIST(w_current->page_clist), row, 1);	
		gtk_clist_moveto (GTK_CLIST(w_current->page_clist), 5, 0, 0.5, 0.5);
		gtk_clist_moveto (GTK_CLIST(w_current->page_clist), 5, 0, 0.5, 0.5);
		gtk_clist_moveto (GTK_CLIST(w_current->page_clist), 5, 0, 0.5, 0.5);
		...
		gtk_signal_connect (GTK_OBJECT (buttonupdatewin), "clicked",
			GTK_SIGNAL_FUNC(update_page_manager), w_current);
