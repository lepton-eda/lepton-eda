...
(begin-test 'geda:page->string
  (let ((A (geda:make-page "/test/page/A"))
    ...
    ;; Pages with identical content
    (assert-equal (geda:page->string A) (geda:page->string B))

    ;; Pages with different content
    (set-line! y '(0 . 0) '(2 . 2))
    (assert-true (not (equal? (geda:page->string A) (geda:page->string B))))
    ))
...
(begin-test 'geda:string->page
  (let ((A (geda:make-page "/test/page/A"))
    ...
    (let* ((B (geda:string->page "/test/page/B" (geda:page->string A))))
      (assert-equal "/test/page/B" (geda:page-filename B))
      (assert-equal 1 (length (geda:page-contents B)))
      (assert-equal (line-info x) (line-info (car (geda:page-contents B)))))))
