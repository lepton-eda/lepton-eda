...
(begin-test 'geda:paths
  (let* ((a (geda:make-path))
         (b (geda:make-path 21)))

    (assert-equal 'path (geda:object-type a))
    (assert-true (geda:path? a))
    (assert-equal 0 (geda:path-length a))
    (assert-thrown 'out-of-range (geda:path-ref a 0))
    (assert-thrown 'out-of-range (geda:path-remove! a 0))
    (assert-equal #f (geda:object-bounds a))

    ; Most trivial path possible
    (assert-equal a (geda:path-insert! a -1 'lineto '(0 . 0)))
    (assert-equal 1 (geda:path-length a))
    (assert-equal '(lineto (0 . 0)) (geda:path-ref a 0))
    (assert-equal '((-8 . 8) . (8 . -8)) (geda:object-bounds a))

    ; Add a move to the start
    (assert-equal a (geda:path-insert! a 0 'moveto '(1 . 0)))
    (assert-equal 2 (geda:path-length a))
    (assert-equal '(moveto (1 . 0)) (geda:path-ref a 0))
    (assert-equal '(lineto (0 . 0)) (geda:path-ref a 1))
    (assert-equal '((-8 . 8) . (9 . -8)) (geda:object-bounds a))

    ; Remove the line
    (assert-equal a (geda:path-remove! a 1))
    (assert-equal 1 (geda:path-length a))
    (assert-thrown 'out-of-range (geda:path-ref a 1))
    (assert-thrown 'out-of-range (geda:path-remove! a 1))
    (assert-equal '((-7 . 8) . (9 . -8)) (geda:object-bounds a))

    ; Add a line, a curve and a closepath.
    (assert-equal a (geda:path-insert! a -1 'lineto '(1 . 2)))
    (assert-equal '(lineto (1 . 2)) (geda:path-ref a 1))
    (assert-equal a (geda:path-insert! a -1 'curveto
                                  '(3 . 4) '(5 . 6) '(7 . 8)))
    (assert-equal '(curveto (3 . 4) (5 . 6) (7 . 8)) (geda:path-ref a 2))
    (assert-equal a (geda:path-insert! a -1 'closepath))
    (assert-equal '(closepath) (geda:path-ref a 3))

    (assert-equal a (geda:path-remove! a 1))

    ; Bad path element type
    (assert-thrown 'misc-error (geda:path-insert! a -1 'BAD-VALUE))

    ; Color
    (assert-equal 21 (geda:object-color b))
))
