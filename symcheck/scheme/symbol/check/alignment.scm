...
(define (check-pin-alignment pin)
  ...
  (define (check-on-grid pin msg-type coord)
    ...
    (let ((num (if (eq? msg-type 'error) 1 2)))
      (or (is-on-grid? coord)
          (blame-object pin
                        msg-type
                        (format #f
                                (_ "Found offgrid pin at location (x~A=~A,y~A=~A)\n")
                                num
                                (x coord)
                                num
                                (y coord))))))
  ...
  (check-on-grid pin 'error (line-start pin))
  (check-on-grid pin 'warning (line-end pin)))
