...
(test-group-with-cleanup "lookup-footprint multiple directories"
  ...
  (let ((footprint-x (lookup-footprint "." "X"))
        (footprint-x.fp (lookup-footprint "." "X.fp")))
    (test-equal footprint-x "./y/c/z/X.fp")
    (test-equal footprint-x.fp "./y/c/z/X.fp"))

  (chmod "y/c/z" #o000)
  (test-equal (lookup-footprint "." "X.fp") "./y/b/X.fp")
  (chmod "y/b" #o000)
  (test-equal (lookup-footprint "." "X.fp") "./y/a/X.fp")
  (chmod "y/a" #o000)
  (test-equal (lookup-footprint "." "X.fp") "./x/b/X.fp")
  (chmod "x/b" #o000)
  (test-equal (lookup-footprint "." "X.fp") "./x/a/X.fp")
  (chmod "x/a" #o000)
  (test-assert (not (lookup-footprint "." "X.fp")))
